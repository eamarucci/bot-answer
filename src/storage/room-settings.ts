...
export function getEffectiveSystemPrompt(roomId: string): string {
  const settings = getRoomSettings(roomId);
  const basePrompt = config.llm.baseSystemPrompt;
  const customPrompt = settings.systemPrompt || config.llm.defaultSystemPrompt;
...
export function getCustomSystemPrompt(roomId: string): string {
  const settings = getRoomSettings(roomId);
  return settings.systemPrompt || config.llm.defaultSystemPrompt;
}
